...
class Expand(nn.Module):
    def __init__(self, in_channel, out_channel, dpeth, heigth, width, kernel_size):
        super(Expand, self).__init__()

        self.upsamp = nn.Upsample(scale_factor=2, mode='nearest')  # scale_factor means division in number of times
        self.conv = nn.Conv3d(in_channel, out_channel >> 1, dpeth, heigth, width, kernel_size, padding=1)
        self.conv1 = nn.Conv3d(in_channel, out_channel >> 1, dpeth, heigth, width, kernel_size)
        self.conv2 = nn.Conv3d(out_channel >> 1, out_channel >> 1, dpeth, heigth, width, kernel_size, )
        self.normalization = nn.BatchNorm3d(out_channel >> 1)
