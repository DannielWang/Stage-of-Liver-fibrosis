...
def read_memmap(filename):
    ...
    dim = _get_dim(header)
    return np.memmap(data_filename, dtype=dtype, mode='r', shape=tuple(dim), offset=offset), header
...
def read(filename):
    ...
    if data_is_compressed:
        ...
        if False:#which('pigz'): # No pigz for reading
            from subprocess import Popen, PIPE
            p = Popen('pigz -dc'.split(), stdout=PIPE, stdin=PIPE, stderr=PIPE)
            data = p.communicate(input=data)[0]
        else:
...
def write(filename, image, header={}):
    ...
    if (compress_data):
        ...
        if which('pigz'):
            from subprocess import Popen, PIPE
            p = Popen('pigz -zc'.split(), stdout=PIPE, stdin=PIPE, stderr=PIPE)
            data = p.communicate(input=data)[0]
        else:
